...
if ((_uid == "76561197994306240") or (_isLoggedInAdmin) or (hasInterface and isServer)) then 
{
	...
	_speedNotifyText = format ["Die Zeit läuft jetzt mit %1x Geschwindigkeit.", _paramTimeSpeed];
	[_speedNotifyText] remoteExec ["hint"];
...
{
	hint "Nur Admins oder Hosts können das Event starten.";
};